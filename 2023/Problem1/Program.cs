...
var spelledOutList = new List<(string SpelledOut, string Ditgit)>
{
    ( "one", "1" ),
    ( "two", "2" ),
    ( "three", "3" ),
...
{
    var inputLines = INPUT_STRING.Split(Environment.NewLine, StringSplitOptions.RemoveEmptyEntries);

    var sum = inputLines
        .Select(line =>
        {
            var digits = new List<string>();
            var wordCache = string.Empty;
            foreach (var item in line)
            {
                if (char.IsDigit(item))
                {
                    digits.Add(item.ToString());
                    wordCache = string.Empty;
                }
                else
                {
                    wordCache += item.ToString();
                    var foundSpelledWord = spelledOutList.Find(x => wordCache.Contains(x.SpelledOut));
                    if (foundSpelledWord != default)
                    {
                        digits.Add(foundSpelledWord.Ditgit);
                        wordCache = wordCache[^1..];
                    }
                }
            }
