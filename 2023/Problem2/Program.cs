...
const string ExampleGameHistoryString = @"
...
const string GameHistoryString = @"
...
var gameHistory = GameRecord.Parse(GameHistoryString);

var possibleGames = gameHistory
    .Where(record => record.IsValid(redLimit, greenLimit, blueLimit))
    .ToList();

foreach (var possibleGame in possibleGames)
{
    Console.WriteLine(possibleGame.GameNumber);
}

Console.WriteLine($"Sum: {possibleGames.Sum(x => x.GameNumber)}");

Console.WriteLine();
Console.WriteLine("PART 2!!!");

var powerSum = gameHistory
    .Select(record =>
    {
        var power = record.GetPower();
        Console.WriteLine($"{record.GameNumber} : {power}");
